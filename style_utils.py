...
def style_transfer(name, content_image, style_image, image_size, style_size, content_layer, content_weight,
    ...
    for t in range(200):
        ...
        content_current = feats[content_layer]
        content_loss_val = content_loss(content_weight, content_current, content_target)
        total_loss += content_loss_val

        # Compute style loss
        style_loss_val = style_loss(feats, style_layers, style_targets, style_weights)
        total_loss += style_loss_val

        # Compute total variation loss for the image
        tv_loss_val = tv_loss(img_var, tv_weight)
        total_loss += tv_loss_val

        # Zero out the gradients for optimizer
        optimizer.zero_grad()

        # Backward pass to compute gradients of all loss terms
        total_loss.backward()
