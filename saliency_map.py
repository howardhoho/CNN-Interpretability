...
# Example with captum
# Computing and visualizing Integrated Gradient (NOTE: Integraded gradients and saliency map are not the same thing, we are providing IG as an example.)
...
# Compute the saliency map for the input tensors with respect to the true labels
attr_saliency = saliency.attribute(X_tensor, target=y_tensor)


# Visualize the saliency maps using the visualize_attr_maps function
visualize_attr_maps('visualization/saliency_captum.png', X, y, class_names, [attr_saliency], ['Saliency Map (Captum)'])
